...
class CNABFileParser(FileParser):
    ...
    def _get_accounting_entries(self, linha_cnab, account_move_line, payment_lines):
        ...
        if linha_cnab["data_credito"] == zeros_date or not linha_cnab["data_credito"]:
            ...
            if bank_name_brcobranca == "sicredi":
                if linha_cnab["codigo_ocorrencia"] in ["02", "09"]:
                    # No Sicredi (CNAB 240),
                    # os códigos de movimento que não possuem Data do Crédito
                    # são os códigos 02 e 09 e portanto a data de crédito
                    # deve ser a mesma da ocorrência.
                    data_credito = self._get_occurrence_date(
                        linha_cnab, date_format, zeros_date
                    )
        else:
        ...
        if valor_tarifa > 0.0 and not self.journal.cnab_return_skip_fee_entry:
            ...
            # Passa a chave 'date' APENAS se houver data_credito válida
            if data_credito:
                row_credito["date"] = data_credito
                row_debito["date"] = data_credito

            row_list.append(row_credito)
            row_list.append(row_debito)
